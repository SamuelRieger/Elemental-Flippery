...
function generateGrid() {
	...
	for (i = 0; i < ((2 * Math.floor((size*size) / 2)) / 2); i++){
		...
		if (difficulty == 1) {
			...
			document.getElementById('instructionText').innerHTML = "Click cards to reveal them. Match cards with the same element symbol.<br><b>Click anywhere to begin, good luck!</b>";
		}
		if (difficulty == 2) {
			...
			document.getElementById('instructionText').innerHTML = "Click cards to reveal them. Match cards with the same element number.<br><b>Click anywhere to begin, good luck!</b>";
		}
		if (difficulty == 3) {
			for (j = 0; j < 2; j++) {
			...
			document.getElementById('instructionText').innerHTML = "Click cards to reveal them. Match cards with their correlating element number and symbol.<br><b>Click anywhere to begin, good luck!</b>";
		}
		/*
		tempRandomStorage.push(rand);
		front.innerHTML = elementList[(group-1)][(rand-1)][1];
...
function disableCards() {
  ...
  if(matches == ((2 * Math.floor((size*size) / 2)) / 2)){
	  document.getElementById('stats').innerHTML = "You completed the grid after <b>" + flips + "</b> pair flips in <b>" + sw.now + "</b> seconds!";
